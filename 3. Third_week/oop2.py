...
class Sofa(Product):
    ...
    def __repr__(self):
        return f'<Sofa name: {self.name}, price: {self.price}, color: {self.color}, texture: {self.texture}, discount: {self.discount}, stock: {self.stock}, max_discount: {self.max_discount}>'

   # def get_color(self):
     #   return f'Цвет обивки: {self.color}, тип ткани: {self.texture}'

sof = Sofa('gg', 11, 'cok', 'te')
print(sof)
